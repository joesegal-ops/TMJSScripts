// ==UserScript==
// @name         Joblogic - PPM Delete Draft Invoices
// @namespace    http://tampermonkey.net/
// @version      1.0.0
// @description  Paste a list of PPM Contract numbers. Lists every DRAFT invoice on each contract and (after you Preview) deletes them, replicating Joblogic's own delete-modal confirmation. Approved invoices and credits are never touched. Always Preview (dry-run) first. Collapses into the shared JL dock.
// @match        https://go.joblogic.com/*
// @grant        none
// @run-at       document-idle
// @downloadURL  https://raw.githubusercontent.com/joesegal-ops/TMJSScripts/main/JL%20PPMs/ppm-delete-draft-invoices.user.js
// @updateURL    https://raw.githubusercontent.com/joesegal-ops/TMJSScripts/main/JL%20PPMs/ppm-delete-draft-invoices.user.js
// ==/UserScript==
...
(function () {
    ...
    const VERSION = '1.0.0';
    const SCRIPT_ID = 'ppm-delete-draft-invoices';
    const SCRIPT_LABEL = '🗑️ PPM Delete Drafts';
    const SCRIPT_COLOR = '#b5372e';
    const SCRIPT_DESC = 'Paste PPM Contract numbers. Lists every DRAFT invoice on each contract; after you Preview and review, deletes them (replicating Joblogic\'s own delete-modal confirmation). Approved invoices and credits are never touched. Always Preview first — deletion cannot be undone.';

    // Throttle to stay under the Joblogic WAF rate limit.
    const DELAY_BETWEEN_INVOICES = 500;
    ...
    let running = false;
    let plan = null; // last computed preview: [{ contract, invoices:[{gid, tag, site, date, order, total}] }]
    ...
    // Delete one draft PPM invoice by GUID, faithfully replicating the /PPMInvoice/DeleteModal
    // confirmation form (Id + ticked agreeCheckbox + fresh antiforgery token -> POST /api/PPMInvoice/Delete).
    async function deleteInvoice(gid) {
        // 1) Load the delete-confirmation modal for this invoice (also yields a fresh token).
        const modalUrl = '/PPMInvoice/DeleteModal?id=' + encodeURIComponent(gid);
        const html = await (await fetchWithRetry(modalUrl, { headers: { 'X-Requested-With': 'XMLHttpRequest' } })).text();
        const doc = new DOMParser().parseFromString(html, 'text/html');
        const form = doc.querySelector('form[action*="PPMInvoice/Delete"]') || doc.querySelector('form[data-ajax="true"]') || doc.querySelector('form');
        if (!form) throw new Error('delete modal form not found (invoice may already be gone or not deletable)');

        // Confirm the modal is actually about THIS invoice before we post.
        const idField = form.querySelector('input[name="Id"]');
        const modalId = idField ? String(idField.value || '') : '';
        if (modalId && modalId.toLowerCase() !== String(gid).toLowerCase()) {
            throw new Error('delete modal Id (' + modalId + ') != requested invoice — aborting for safety');
        }

        // 2) Serialise the form exactly as the browser would, with agreeCheckbox ticked.
        const csrf = (form.querySelector('input[name="__RequestVerificationToken"]') || {}).value || getToken();
        const params = new URLSearchParams();
        form.querySelectorAll('input[name], select[name], textarea[name]').forEach(el => {
            const name = el.name;
            if (name === '__RequestVerificationToken') return; // sent as header instead
            if (el.type === 'checkbox') {
                // The UI requires ticking "I agree" before the Delete button enables. Send it as checked.
                if (name === 'agreeCheckbox') params.append(name, el.value || 'on');
                return;
            }
            if (el.type === 'submit' || el.type === 'button') return;
            params.append(name, el.value == null ? '' : el.value);
        });
        if (!params.has('agreeCheckbox')) params.append('agreeCheckbox', 'on');
        if (!params.has('Id')) params.append('Id', gid);

        // 3) POST the delete.
        const r = await fetchWithRetry('/api/PPMInvoice/Delete', {
            method: 'POST',
            referrer: location.origin + '/PPMInvoice/Detail/' + gid,
        ...
        if (!r.ok) throw new Error('Delete HTTP ' + r.status + ': ' + txt.slice(0, 200));
        let j = null;
        try { j = JSON.parse(txt); } catch (e) {}
        if (j && j.success === false) throw new Error('Delete success=false: ' + (j.Message || j.message || txt.slice(0, 160)));
        // Some deletes return an HTML fragment (redirect/toast) with 200 and no JSON — treat 200 as success.
        return true;
    }
    ...
    // =======================================================================
    function createUI() {
        if (document.getElementById('jl-ppmdel-panel')) return;

        panel = document.createElement('div');
        panel.id = 'jl-ppmdel-panel';
        ...
        title.innerHTML = 'PPM Delete Draft Invoices <span style="font-weight:400;color:#8a8ab5;font-size:11px;">v' + VERSION + '</span>';
        ...
        rules.style.cssText = 'background:#2a1212;border-left:3px solid #b5372e;border-radius:4px;padding:8px 10px;margin-bottom:8px;line-height:1.5;color:#e6c9c5;';
        rules.innerHTML = 'For each contract, lists every <b>DRAFT</b> invoice and deletes it via Joblogic\'s own confirmation flow.<br>' +
            '• Only <b>draft</b>, non-credit PPM invoices on the given contract are touched.<br>' +
            '• <b>Approved invoices and credit notes are never deleted.</b><br>' +
            '• <b style="color:#ff9;">Deletion cannot be undone.</b> Always <b>Preview</b> first and read the list.';

        const lbl = document.createElement('div');
        lbl.style.cssText = 'color:#aaa;margin-bottom:4px;';
        lbl.textContent = 'PPM Contract numbers (one per line — PM numbers, or paste contract URLs / GUIDs):';

        contractInput = document.createElement('textarea');
        contractInput.rows = 6;
        ...
        runBtn = mkBtn('Delete drafts', '#b5372e');
        runBtn.disabled = true; runBtn.style.opacity = '0.5';
        runBtn.addEventListener('click', () => confirmAndDelete());
    ...
    // Short identifying label for an invoice row.
    function invLabel(row, idx) {
        const bits = [];
        if (row.DateRaised) bits.push(String(row.DateRaised).split('T')[0].split(' ')[0]);
        if (row.OrderNumber) bits.push('PO ' + row.OrderNumber);
        const money = row.GrandTotalString || row.GrandTotal || row.TotalIncludingVatString || row.GrandTotalDecimal;
        if (money != null && money !== '') bits.push('' + money);
        return bits.length ? bits.join(' · ') : ('draft ' + (idx + 1));
    }
    ...
    async function run(dryRun) {
        ...
        logArea.innerHTML = '';
        plan = null;
        log('=== PREVIEW (dry run) — nothing will be deleted ===', '#ff0');
        log(terms.length + ' contract(s): ' + terms.join(', '), '#0af');
        log('');

        const stats = { contracts: 0, invoices: 0, errors: 0 };
        const builtPlan = [];

        for (let ci = 0; ci < terms.length; ci++) {
            if (!running) { log('Stopped by user.', '#f55'); break; }
            const term = terms[ci];
            setProgress('Scanning ' + (ci + 1) + '/' + terms.length + ': ' + term);
            ...
            log('  ' + invoices.length + ' draft invoice(s) — WILL BE DELETED:', '#f99');

            const contractPlan = { contract, invoices: [] };
            invoices.forEach((row, ii) => {
                stats.invoices++;
                const label = invLabel(row, ii);
                log('    • ' + label, '#fca');
                contractPlan.invoices.push({ gid: row.UniqueId, label });
            });
        ...
        log('Draft invoices found: ' + stats.invoices, stats.invoices ? '#f99' : '#888');
        log('Lookup errors: ' + stats.errors, stats.errors ? '#f55' : '#888');

        if (stats.invoices > 0) {
            plan = builtPlan;
            runBtn.disabled = false; runBtn.style.opacity = '1';
            setProgress('Preview ready — ' + stats.invoices + ' draft(s) to delete. Review, then "Delete drafts".');
            log('');
            log('⚠ Review the list above. Clicking "Delete drafts" permanently deletes all ' + stats.invoices + ' draft invoice(s).', '#ff0');
        } else {
            setProgress('No draft invoices found.');
        }
    ...
    // =======================================================================
    // Main — delete (only from a computed preview plan)
    // =======================================================================
    function confirmAndDelete() {
        if (!plan) { alert('Run Preview first.'); return; }
        const total = plan.reduce((n, c) => n + c.invoices.length, 0);
        if (!total) { alert('Nothing to delete.'); return; }
        const ok = confirm('Permanently delete ' + total + ' draft PPM invoice(s) across ' + plan.length + ' contract(s)?\n\nThis cannot be undone.');
        if (ok) deletePlan();
    }
    ...
    async function deletePlan() {
        ...
        stopBtn.style.display = 'inline-block';
        logArea.innerHTML = '';
        log('=== DELETING DRAFT INVOICES ===', '#f66');
        log('');
        ...
        for (const cp of plan) {
            if (!running) { log('Stopped by user.', '#f55'); break; }
            log('▸ ' + cp.contract.number + (cp.contract.site ? '  · ' + cp.contract.site : ''), '#fff');
            for (let ii = 0; ii < cp.invoices.length; ii++) {
                if (!running) break;
                const inv = cp.invoices[ii];
                setProgress('Deleting ' + (stats.deleted + 1) + '/' + plan.reduce((n, c) => n + c.invoices.length, 0));
                try {
                    await deleteInvoice(inv.gid);
                    log('    ✓ deleted — ' + inv.label, '#0fa');
                    stats.deleted++;
                } catch (e) {
                    log('    ✗ ' + inv.label + ' — ' + e.message, '#f55');
                    stats.errors++;
                }
        ...
        log('');
        log('===== SUMMARY =====', '#0af');
        log('Deleted: ' + stats.deleted, stats.deleted ? '#0fa' : '#888');
        log('Errors: ' + stats.errors, stats.errors ? '#f55' : '#888');
        setProgress('Done. Deleted ' + stats.deleted + ', errors ' + stats.errors + '.');

        // Invalidate the plan — the invoices are gone; force a fresh Preview before any further delete.
        plan = null;
        runBtn.disabled = true; runBtn.style.opacity = '0.5';
        running = false;
        previewBtn.disabled = false; previewBtn.style.opacity = '1';
