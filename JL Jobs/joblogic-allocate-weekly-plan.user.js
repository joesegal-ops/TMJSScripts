...
// @downloadURL  https://raw.githubusercontent.com/joesegal-ops/TMJSScripts/main/JL%20Jobs/joblogic-allocate-weekly-plan.user.js
// @updateURL    https://raw.githubusercontent.com/joesegal-ops/TMJSScripts/main/JL%20Jobs/joblogic-allocate-weekly-plan.user.js
// ==/UserScript==
...
(function () {
    ...
    if (window.__jlWeeklyPlanLoaded) {
        console.log('[WeeklyPlan] already loaded — skipping second init');
        return;
    }
    window.__jlWeeklyPlanLoaded = true;

    // ===========================================================================
    // CONFIG
    // ===========================================================================
    const STATE_KEY = 'jl-weeklyplan-state-v1';
    const LOG_KEY   = 'jl-weeklyplan-log-v1';
    ...
    // ---------------------------------------------------------------------------
    // WEEKLY PLAN PARSING (free text -> one row per job per listed day)
    // ---------------------------------------------------------------------------
    const WEEKDAYS = [
        { idx: 0, re: /\bmon(?:day)?\b/i },
        { idx: 1, re: /\btue(?:s|sday)?\b/i },
        { idx: 2, re: /\bwed(?:nesday)?\b/i },
        { idx: 3, re: /\bthu(?:r|rs|rsday)?\b/i },
        { idx: 4, re: /\bfri(?:day)?\b/i },
        { idx: 5, re: /\bsat(?:urday)?\b/i },
        { idx: 6, re: /\bsun(?:day)?\b/i }
    ];
    const WEEKDAY_NAMES = ['Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat', 'Sun'];
    // Job-reference prefixes seen across PPM (PM…/nnn) and reactive (RE…) jobs,
    ...
    // Monday (weekday index 0) of the week containing date d.
    ...
    function jlDateStr(d, hhmm) {
        const pad = (n) => String(n).padStart(2, '0');
        return `${pad(d.getDate())}/${pad(d.getMonth() + 1)}/${d.getFullYear()} ${hhmm}`;
    }

    // Map a line's wording to an appointment start time.
    function timeForLine(line) {
        if (/early\s*morning/i.test(line)) return '06:00';
        if (/\bmorning\b/i.test(line))     return '08:00';
        if (/\bafternoon\b/i.test(line))   return '12:00';
        if (/\bevening\b/i.test(line))     return '16:00';
        return DEFAULT_APPT_TIME;
    }

    // Weekday indices mentioned in a line, in order of first appearance, deduped.
    function daysInLine(line) {
        const hits = [];
        for (const wd of WEEKDAYS) {
            wd.re.lastIndex = 0;
            const m = wd.re.exec(line);
            if (m) hits.push({ idx: wd.idx, at: m.index });
        }
        hits.sort((a, b) => a.at - b.at);
        const seen = new Set(), out = [];
        for (const h of hits) if (!seen.has(h.idx)) { seen.add(h.idx); out.push(h.idx); }
        return out;
    }
    ...
    function refsInLine(line) {
        ...
        REF_RE.lastIndex = 0;
        let m;
        while ((m = REF_RE.exec(line)) !== null) out.push(m[1].replace(/\s+/g, '').toUpperCase());
        return out;
    }

    // An engineer-header line: no weekday, no job ref, reads like a person name.
    function looksLikeName(line) {
        const s = line.replace(/^[\s\-•*]+/, '').trim();
        if (!s) return false;
        if (daysInLine(s).length) return false;
        if (refsInLine(s).length) return false;
        const words = s.split(/\s+/);
        if (words.length < 1 || words.length > 4) return false;
        return /^[A-Za-z][A-Za-z' .\-]*$/.test(s);
    }

    // Free text + a week-start date -> { rows, skipped }.
    //   rows:    one per (job ref × listed weekday) with a computed dateStr.
    //   skipped: day/job lines that named no job reference (can't be allocated).
    function parseWeeklyPlan(text, weekStartRaw) {
        const anchor = parseDMY(weekStartRaw);
        const monday = anchor ? mondayOfWeek(anchor) : null;
        const rows = [], skipped = [];
        let currentEngineer = '';
        for (const rawLine of String(text || '').split(/\r?\n/)) {
            const line = rawLine.trim();
            if (!line) continue;
            if (looksLikeName(line)) { currentEngineer = line.replace(/^[\s\-•*]+/, '').trim(); continue; }

            const refs = refsInLine(line);
            const days = daysInLine(line);
            if (!refs.length) {
                // A line that named a day but carried no job number — surface it.
                if (days.length) skipped.push(line);
                continue;
            }
            const ref = refs[0];
            const useDays = days.length ? days : [0]; // ref but no day → Monday
            const hhmm = timeForLine(line);
            const desc = line.replace(/^[\s\-•*]+/, '')
                             .replace(REF_RE, '')
                             .replace(/\s{2,}/g, ' ')
                             .replace(/[\s,.:;\-]+$/, '')
                             .trim();
            for (const dayIdx of useDays) {
                let dateStr = null;
                if (monday) {
                    const d = new Date(monday.getFullYear(), monday.getMonth(), monday.getDate() + dayIdx);
                    dateStr = jlDateStr(d, hhmm);
                }
                rows.push({
                    ref,
                    dateStr,
                    rawDate: dateStr || '',
                    engineerRaw: currentEngineer,
                    dayLabel: WEEKDAY_NAMES[dayIdx],
                    description: desc
                });
            }
        }
    ...
    function createUI() {
        if (document.getElementById('jl-weeklyplan-panel')) return;

        panelEl = document.createElement('div');
        panelEl.id = 'jl-weeklyplan-panel';
        panelEl.innerHTML = `
<style>
  #jl-weeklyplan-panel{position:fixed;top:10px;right:10px;z-index:99999;background:#1a1a2e;color:#eee;
  ...
  #jl-weeklyplan-panel header{display:flex;justify-content:space-between;align-items:center;
    padding:10px 14px;border-bottom:1px solid #333;cursor:move;user-select:none;}
  #jl-weeklyplan-panel header b{font-size:14px}
  #jl-weeklyplan-panel header .close{background:none;border:none;color:#eee;font-size:16px;cursor:pointer}
  #jl-weeklyplan-panel .body{padding:10px 14px;display:flex;flex-direction:column;gap:8px;
    overflow-y:auto;}
  #jl-weeklyplan-panel .progress{color:#0fa;font-weight:600}
  #jl-weeklyplan-panel .controls{display:flex;gap:6px;flex-wrap:wrap}
  #jl-weeklyplan-panel button{background:#2563eb;color:#fff;border:0;border-radius:4px;padding:7px 12px;
    cursor:pointer;font-family:inherit;font-size:12px;}
  #jl-weeklyplan-panel button.paste{background:#08a}
  #jl-weeklyplan-panel button.run-report{background:#ca8a04}
  #jl-weeklyplan-panel button.run-allocate{background:#0a8}
  #jl-weeklyplan-panel button.stop{background:#a22}
  #jl-weeklyplan-panel button.report{background:#648}
  #jl-weeklyplan-panel button.clear{background:#555}
  #jl-weeklyplan-panel button.capture{background:#9333ea}
  #jl-weeklyplan-panel button[disabled]{opacity:.45;cursor:not-allowed}
  #jl-weeklyplan-panel .defaults{display:flex;flex-wrap:wrap;gap:6px 12px;align-items:center;
    background:#13132a;padding:8px 10px;border-radius:4px;}
  #jl-weeklyplan-panel .defaults label{display:inline-flex;flex-direction:column;gap:3px;font-size:11px;
    color:#9ca3af;}
  #jl-weeklyplan-panel .defaults input{background:#0a0a1a;color:#eee;border:1px solid #333;
    border-radius:3px;padding:4px 6px;font:inherit;width:200px;}
  #jl-weeklyplan-panel .defaults .default-hint{color:#6b7280;font-size:10.5px;flex:1;min-width:160px;}
  #jl-weeklyplan-panel .log{background:#0a0a1a;padding:8px;border-radius:4px;flex:1;overflow-y:auto;
...
<div class="body">
  <div class="progress">Paste the weekly plan and set the week-start date.</div>
  <div class="defaults">
    <span class="default-hint">Click <b>Paste Plan</b> to paste the engineer/day text and the week-start date.
    Each job line is expanded to one visit per listed day. "Run Report" surveys read-only;
    "Run Allocate" commits. Existing visits are never cancelled.</span>
  </div>
  <div class="controls">
    <button class="paste">Paste Plan</button>
  ...
  <div class="log"></div>
        ...
        panelEl.querySelector('.close').addEventListener('click', () => { panelEl.style.display = 'none'; });
    ...
    const DEFAULTS_KEY = 'jl-weeklyplan-defaults-v1';
    ...
    const LOG_LINES_KEY = 'jl-weeklyplan-loglines-v1';
    ...
    // Paste dialog
    // ---------------------------------------------------------------------------
    const WEEKSTART_KEY = DEFAULTS_KEY + ':weekstart';

    function openPasteDialog() {
        const savedWeek = (() => { try { return localStorage.getItem(WEEKSTART_KEY) || ''; } catch { return ''; } })();
        const overlay = document.createElement('div');
        overlay.style.cssText = 'position:fixed;inset:0;background:rgba(0,0,0,.6);z-index:100000;' +
            'display:flex;align-items:center;justify-content:center;';
        overlay.innerHTML = `
<div style="background:#fff;color:#111;border-radius:8px;width:680px;max-width:92vw;
  ...
  <div style="padding:14px 16px;">
    ...
    <textarea id="jl-paste-ta" style="width:100%;height:260px;font:13px monospace;padding:8px;
      ...
      placeholder="Paste the plan. Lines that are just a name set the engineer; day-of-week lines with a PM/RE job number become visits.

Example:
Callum Holdstock
- Monday/Wednesday: St Katharine's Way, pressure leak test. PM0001389/002
- Thursday/Friday: refrigerant charge confirmation. PM0001389/002"></textarea>
    <div style="color:#6b7280;font-size:12px;margin-top:6px;">
      One visit per listed day. Times: "Early Morning" 06:00, "Morning" 08:00,
      "Afternoon" 12:00, else ${DEFAULT_APPT_TIME}. Duration ${DEFAULT_DURATION_MINS} min.
    </div>
    ...
    <div id="jl-paste-preview" style="color:#374151;font-size:11px;margin-top:6px;
      max-height:160px;overflow-y:auto;font-family:monospace;"></div>
    <div style="text-align:right;margin-top:10px;">
      ...
      <button id="jl-paste-ok" style="background:#2563eb;color:#fff;border:0;border-radius:4px;
        ...
        const week = overlay.querySelector('#jl-paste-week');
        const count = overlay.querySelector('#jl-paste-count');
        const preview = overlay.querySelector('#jl-paste-preview');

        const refresh = () => {
            const weekOk = !!parseDMY(week.value);
            week.style.borderColor = week.value && !weekOk ? '#dc2626' : '#d1d5db';
            const { rows, skipped } = parseWeeklyPlan(ta.value, week.value);
            const bad = rows.filter(r => !r.engineerRaw);
            count.textContent = `${rows.length} visit${rows.length === 1 ? '' : 's'} detected`
                + (bad.length ? ` — ${bad.length} with no engineer` : '')
                + (skipped.length ? ` — ${skipped.length} line(s) skipped (no job ref)` : '')
                + (week.value && !weekOk ? ' — invalid week-start date' : '');
            count.style.color = (bad.length || (week.value && !weekOk)) ? '#dc2626' : '#2563eb';
            const rowsHtml = rows.slice(0, 12).map(r => {
                const ok = r.dateStr && r.engineerRaw;
                return `<div style="color:${ok ? '#374151' : '#dc2626'};">${r.engineerRaw || '[no engineer]'} | ${r.dayLabel} ${r.dateStr || '[set week start]'} | ${r.ref}</div>`;
            }).join('') + (rows.length > 12 ? `<div style="color:#9ca3af;">...and ${rows.length - 12} more</div>` : '');
            const skipHtml = skipped.length
                ? `<div style="color:#b45309;margin-top:6px;">Skipped (no job number): ${skipped.slice(0, 4).map(escapeHtml).join(' · ')}${skipped.length > 4 ? ' …' : ''}</div>`
                : '';
            preview.innerHTML = rowsHtml + skipHtml;
        };
        ta.addEventListener('input', refresh);
        week.addEventListener('input', () => {
            try { localStorage.setItem(WEEKSTART_KEY, week.value); } catch {}
            refresh();
        });
        refresh();
        overlay.querySelector('#jl-paste-cancel').onclick = () => overlay.remove();
        overlay.querySelector('#jl-paste-ok').onclick = () => {
            if (!parseDMY(week.value)) { week.style.borderColor = '#dc2626'; week.focus(); return; }
            const { rows, skipped } = parseWeeklyPlan(ta.value, week.value);
            const keep = rows.filter(r => r.ref);
            overlay.remove();
            if (!keep.length) { setProgress('No visits found in the plan.'); return; }
            const st = {
                running: false,
                dryRun: true,
                rows: keep.map(r => ({ ...r, internalId: null, error: null, status: 'pending' })),
                currentIndex: 0,
                phase: 'idle'
            };
            saveState(st);
            ...
            if (logArea) logArea.innerHTML = '';
            renderFromState();
            const noEng = keep.filter(r => !r.engineerRaw).length;
            log(`Loaded ${keep.length} visit(s) from the plan.`, '#0af');
            if (skipped.length) {
                log(`⚠ ${skipped.length} line(s) had a day but NO job number — skipped (not scheduled):`, '#fa0');
                skipped.forEach(s => log(`  · ${s}`, '#fa0'));
            }
            if (noEng) {
                log(`${noEng} visit(s) have no engineer (no name line above them) — fix the plan or they will fail.`, '#fa0');
    ...
    function onResetClick() {
        ...
        clearLogLines();
        if (logArea) logArea.innerHTML = '';
        setProgress('Cleared. Paste the weekly plan to begin.');
    ...
    async function onStartClick(reportOnly) {
        const st = loadState();
        if (!st || !st.rows?.length) { setProgress('Nothing loaded — paste the weekly plan first.'); return; }

        // Validate: every row must have an engineer (set by a name line in the
        // plan) and a date (computed from the week-start). Both come from the
        // parser, so a gap here means the plan was malformed.
        const missing = st.rows.filter(r => !r.engineerRaw);
        if (missing.length) {
            log(`Cannot start — ${missing.length} visit(s) have no engineer (add a name line above those jobs in the plan).`, '#f55');
            missing.slice(0, 5).forEach(m => log(`  · ${m.ref}`, '#f55'));
            saveState(st);
            return;
        }
        const noDate = st.rows.filter(r => !r.dateStr);
        if (noDate.length) {
            log(`Cannot start — ${noDate.length} visit(s) have no date. Re-load the plan with a valid week-start date.`, '#f55');
            saveState(st);
            return;
        }
        if (!reportOnly) {
            const ok = confirm(`Run Allocate will, for each of the ${st.rows.length} visit(s):\n  • Open the job's Visits tab\n  • Allocate the matched engineer on the computed date\n\nExisting visits are NOT cancelled or changed — this only ADDS visits.\n\nProceed?`);
            if (!ok) {
                st.running = false;
                saveState(st);
                setRunningUI(false);
    ...
    async function handleCurrentJob(st) {
        ...
        try {
            ...
            // Weekly-plan mode is purely additive: never cancel existing visits.
            const toCancel = [];
            row.cancelTargets = [];
...
<div style="background:#fff;color:#111;border-radius:8px;width:94vw;max-width:1280px;max-height:92vh;
  ...
  <div style="padding:12px 16px;background:#111827;color:#f9fafb;display:flex;justify-content:space-between;align-items:center;">
    <strong>Weekly Plan Allocation — Report</strong>
    <div>
  ...
  <div style="padding:12px 16px;overflow-y:auto;">
    <table style="width:100%;border-collapse:collapse;font-size:12px;">
      <thead>
        <tr style="background:#f3f4f6;text-align:left;">
          ...
          <th style="padding:6px;border-bottom:1px solid #d1d5db;">Plan engineer</th>
    ...
    function renderFromState() {
        ...
        if (!st) {
            setRunButtonsEnabled(false);
            setProgress('Paste the weekly plan and set the week-start date to begin.');
            setRunningUI(false);
            return;
        }
